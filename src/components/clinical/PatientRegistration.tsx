...
const patientSchema = z.object({
    ...
    insuranceMembershipNo: z.string().optional(),
...
const PatientRegistration: React.FC = () => {
    ...
    const loadInsuranceOptions = debounce(async (inputValue, callback) => {
        try {
            const res = await api.getInsurances({ search: inputValue });

            const options = res.data.map((insurance) => ({
                value: insurance.insurance_id,
                label: insurance.insurance_name,
            }));

            setInsuranceOptions(options); // So you can resolve selected value
            callback(options);
        } catch (err) {
    ...
    const loadCompanyOptions = debounce(async (inputValue, callback) => {
        try {
            const res = await api.getCompanies({search: inputValue});
            ...
            setCompanyOptions(options); // Store for value matching
            callback(options);
        } catch (err) {
