...
import { Building, MapPin, Check, AlertCircle, Loader2, ChevronDown } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useSettings } from '../../context/SettingsContext';
import { useToast } from '../../context/ToastContext';

const IntelligentBranchSwitcher: React.FC = () => {
  ...
  // Check if user is a doctor (for special handling)
  ...
  // Don't show dropdown functionality if user only has access to one branch
  const canSwitchBranches = user && allowedBranches.length > 1;
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        onClick={() => canSwitchBranches && setIsOpen(!isOpen)}
        disabled={!canSwitchBranches}
        title={canSwitchBranches ? "Click to switch branch" : "You only have access to this branch"}
      >
        <div className="flex items-center text-[#2B3990]">
          <Building className={`w-5 h-5 mr-2 ${isDoctor ? 'text-blue-600' : ''}`} />
          <span className="font-bold text-xl">{currentBranch.name.toUpperCase()}</span>
          {canSwitchBranches && (
            <ChevronDown className={`w-4 h-4 ml-1 transition-transform duration-200 ${isOpen ? 'transform rotate-180' : ''} ${isDoctor ? 'text-blue-600' : ''}`} />
          )}
          {isDoctor && canSwitchBranches && (
      ...
      {isOpen && canSwitchBranches && (
        <>
          ...
          />
          <div className="absolute left-0 mt-1 w-64 bg-white rounded-lg shadow-xl z-[1000] border border-gray-200 overflow-hidden">
            <div className={`px-4 py-3 ${isDoctor ? 'bg-blue-50' : 'bg-gray-50'} border-b border-gray-100`}>
              <h3 className="text-sm font-semibold text-gray-700">Switch Branch</h3>
              <p className="text-xs text-gray-500 mt-1">
                {isDoctor
                  ? "As a doctor, you can access patients across all branches"
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors ${
                    branch.id === user?.branch
                      ? isDoctor ? 'bg-blue-100' : 'bg-blue-50'
                      : isDoctor && branch.allowRemoteAccess ? 'bg-blue-50/30' : ''
                  }`}
...
export default IntelligentBranchSwitcher;
