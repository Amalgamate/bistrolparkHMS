...
router.get('/departments', async (req, res) => {
  try {
    ...
    try {
      // Query the department_summary view we created
...
// Get ward overview for Bristol Park Hospital
router.get('/wards', async (req, res) => {
  try {
    ...
    try {
      ...
      let query = `
        SELECT * FROM bristol_park_overview
      `;

      const params = [];
      if (department) {
        query += ` WHERE ward_type = $1`;
        params.push(department);
      }

      query += ` ORDER BY ward_type, ward_id`;
