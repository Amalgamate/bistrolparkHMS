...
# Configuration
SOURCE_DB_NAME = "bristol_park_legacy"  # Your source database name
...
def clean_target_database():
    """Clean up the target database by dropping all tables"""
    conn = connect_to_db(TARGET_DB_NAME)
    if not conn:
        return False
    ...
    try:
        logging.info("Cleaning up target database...")

        # Get all tables in the target database
        tables = list_tables(conn)

        if not tables:
            logging.info("No tables to clean up in target database.")
            cursor.close()
            conn.close()
            return True
        ...
        logging.info("✅ Target database cleaned successfully.")
        cursor.close()
        conn.close()
        return True
    except psycopg2.Error as e:
        logging.error(f"❌ Error cleaning target database: {e}")
        cursor.close()
        conn.close()
        return False
...
def sort_tables_by_dependencies(tables, dependencies):
    ...
    if circular_deps:
        logging.warning(f"⚠️ Circular dependencies detected for tables: {', '.join(circular_deps)}")
        logging.warning("These tables will be migrated but foreign key constraints might fail")

    # Add any tables that weren't visited due to circular dependencies
    for table in tables:
        if table not in result:
            result.append(table)
            logging.warning(f"⚠️ Adding table with circular dependency to the end: {table}")
...
def get_table_schema(conn, table_name):
    ...
    try:
        ...
        # Get column definitions - simpler query to avoid issues
        cursor.execute("""
            SELECT
                column_name,
                data_type,
        ...
        # Process columns to add is_identity flag
        processed_columns = []
        for col in columns:
            column_name, data_type, char_max_length, is_nullable, default = col
            # Check if it's an identity column (auto-increment)
            is_identity = default is not None and 'nextval' in str(default).lower()
            processed_columns.append((column_name, data_type, char_max_length, None, None, is_nullable, default, is_identity))
        ...
        logging.error(f"❌ Unexpected error getting schema for table {table_name}: {e}")
        cursor.close()
        return None
...
def generate_create_table_sql(table_name, schema):
    ...
    for column in schema['columns']:
        column_name, data_type, char_max_length, num_precision, num_scale, is_nullable, default, is_identity = column

        # Track identity columns
        if is_identity:
            identity_columns.append(column_name)
        ...
        # Only add default if it's not an identity column
        if default is not None and not is_identity:
    ...
    return create_table_sql, fk_constraints, identity_columns
...
def insert_data_into_target_db(table_name, column_names, rows, identity_columns=None):
    ...
    try:
        ...
        # Prepare the INSERT statement with OVERRIDING SYSTEM VALUE if needed
        if identity_columns and any(col in column_names for col in identity_columns):
...
def migrate_table(source_conn, table_name):
    ...
    create_sql, fk_constraints, identity_columns = generate_create_table_sql(table_name, schema)
    ...
    if not insert_data_into_target_db(table_name, data['column_names'], data['rows'], identity_columns):
...
def main():
    ...
    # Step 2: Clean up the target database
    if not clean_target_database():
        logging.error("❌ Failed to clean up target database. Aborting.")
        return

    # Step 3: Create the UUID extension
    ...
    # Step 7: Prepare tables for migration
    ...
    # Step 8: Sort tables by dependencies to ensure proper migration order
    ...
    for table in patient_tables:
        ...
        if schema:
            ...
            logging.warning(f"⚠️ Skipping table {table} due to schema retrieval error")

    # Sort tables based on dependencies
    sorted_tables = sort_tables_by_dependencies(valid_tables, TABLE_DEPENDENCIES)
    ...
    # Step 9: Migrate tables in the correct order
    ...
    if MIGRATED_ROWS > 0 and total_elapsed_time > 0:
